...
BEGIN_MESSAGE_MAP(WinMTRDialog, CDialog)
	...
	ON_BN_CLICKED(ID_Export, &WinMTRDialog::OnClickedExport)
	ON_BN_CLICKED(ID_Copyall, &WinMTRDialog::OnClickedCopyall)
	ON_WM_HELPINFO()
END_MESSAGE_MAP()
...
BOOL WinMTRDialog::OnInitDialog()
{
	...
	#ifndef  _WIN64
	char caption[] = {"WinMTR v1.1.1 32 bit"};
	#else
	char caption[] = {"WinMTR v1.1.1 64 bit"};
	#endif
...
void WinMTRDialog::OnClickedExport()
{
	// TODO:  在此添加控件通知处理程序代码
	TCHAR BASED_CODE szFilter[] = _T("Text Files (*.csv)|*.csv|All Files (*.*)|*.*||");
	...
	if (dlg.DoModal() == IDOK) {
		...
		for (int i = 0; i < nh; i++) {
			...
			sprintf(t_buf, "%d,%s,%d,%d,%d,%d,%d,%d,%d\n",
...
void WinMTRDialog::OnClickedCopyall()
{
	// TODO:  在此添加控件通知处理程序代码
	char buf[255], t_buf[1000], f_buf[255 * 100];
		CString ti;
